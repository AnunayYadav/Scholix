...
const FeaturedQuizCard: React.FC<FeaturedQuizCardProps> = ({
  ...
  return (
    <motion.div
      ...
      className="relative overflow-hidden rounded-[32px] border border-slate-200 dark:border-white/5 bg-gradient-to-br from-orange-50 via-white to-amber-50 dark:from-orange-950/10 dark:via-dark-950/60 dark:to-amber-950/10 shadow-xl"
    >
      ...
      <div className="relative p-6 md:p-8">
        <div className="flex flex-col md:flex-row md:items-center gap-6">
          ...
          <div className="flex-1 min-w-0 space-y-4">
            ...
            {/* Quiz name */}
            <h3 className="text-xl md:text-2xl font-semibold text-slate-900 dark:text-white tracking-tight leading-tight">
              {quiz.name}
            </h3>
            ...
            {quiz.units && quiz.units.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {quiz.units.slice(0, 3).map(unit => (
                  <span key={unit} className="px-2 py-0.5 rounded-md bg-slate-100 dark:bg-white/5 text-[10px] font-semibold text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-white/5">
                    {unit}
                  </span>
                ))}
                {quiz.units.length > 3 && (
            ...
            {/* Info line */}
            <div className="flex items-center gap-4 flex-wrap text-[11px] font-semibold text-slate-500">
          ...
          <div className="flex flex-col items-center gap-3 flex-shrink-0">
            {isCompleted ? (
              <div className="flex flex-col items-center gap-2">
                ...
                {completedScore !== null && (
                  <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">{completedScore}%</span>
                )}
              </div>
            ) : (
                <motion.button
