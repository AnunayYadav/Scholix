...
const LeaderboardSection: React.FC<LeaderboardProps> = ({ currentUserId }) => {
    ...
    if (loading) {
        return (
            <div className="glass-panel p-8 rounded-[32px] animate-pulse bg-white/40 dark:bg-white/[0.02]">
                <div className="flex items-center gap-4 mb-8">
                    <div className="w-10 h-10 bg-slate-200 dark:bg-white/5 rounded-2xl" />
                    <div className="h-6 w-32 bg-slate-200 dark:bg-white/5 rounded-full" />
                </div>
    ...
    const visibleLeaderboard = isExpanded ? leaderboard : leaderboard.slice(0, 3);
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="glass-panel overflow-hidden rounded-[40px] border border-slate-200/50 dark:border-white/[0.08] shadow-2xl shadow-slate-200/20 dark:shadow-none bg-white/50 dark:bg-white/[0.02] backdrop-blur-2xl">
                <motion.div layout transition={{ duration: 0.4 }} className="overflow-hidden">
                ...
                {leaderboard.length > 3 && (
                    <div className="p-4 border-t border-slate-100 dark:border-white/[0.05] bg-slate-50/30 dark:bg-white/[0.01]">
                        <button 
                            onClick={() => setIsExpanded(!isExpanded)}
                            className="w-full py-2.5 rounded-2xl flex items-center justify-center gap-2 text-[10px] font-bold text-slate-500 hover:text-orange-600 transition-all uppercase tracking-widest group"
                        >
                            {isExpanded ? (
                                <>
                                    <span>Show Less</span>
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className="w-3 h-3 transition-transform group-hover:-translate-y-0.5"><path d="M18 15l-6-6-6 6"/></svg>
                                </>
                            ) : (
                                <>
                                    <span>View Full Leaderboard</span>
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className="w-3 h-3 transition-transform group-hover:translate-y-0.5"><path d="M6 9l6 6 6-6"/></svg>
                                </>
                            )}
                        </button>
                    </div>
