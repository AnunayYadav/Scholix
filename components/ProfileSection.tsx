    ...
    <div className="max-w-4xl mx-auto space-y-12 animate-fade-in pb-32 px-4 relative">
      ...
      <header className="flex flex-col items-center text-center relative z-10">
        <div className="relative group">
          <div className="relative w-40 h-40 flex items-center justify-center transition-all duration-500 hover:scale-[1.02]">
            ...
            <div 
              className="w-full h-full rounded-full overflow-hidden flex items-center justify-center relative bg-slate-100 dark:bg-white/[0.03] border border-slate-200 dark:border-white/5"
        ...
        <div className="mt-8 space-y-2">
          <h2 className="text-3xl md:text-4xl font-semibold text-slate-800 dark:text-white tracking-tight flex items-center justify-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-12 space-y-8">
          <div className="glass-panel p-8 md:p-10 rounded-[32px] border border-slate-200 dark:border-white/5 bg-white/50 dark:bg-black/20 backdrop-blur-xl">
            <div className="flex items-center justify-between mb-10 pb-6 border-b border-slate-100 dark:border-white/5">
              <div>
                <h3 className="text-lg font-semibold text-slate-800 dark:text-white leading-tight">General Info</h3>
                <p className="text-xs text-slate-500 font-medium">Manage your public presence</p>
              </div>
              <div className="flex items-center gap-4 bg-slate-50 dark:bg-white/[0.03] p-2 pr-4 rounded-2xl border border-slate-100 dark:border-white/5">
                <button
                  onClick={() => setForm({ ...form, is_public: !form.is_public })}
                  className={`relative w-10 h-5 rounded-full transition-all border-none outline-none ${form.is_public ? 'bg-orange-500' : 'bg-slate-300 dark:bg-white/10'}`}
                >
                  <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all shadow-sm ${form.is_public ? 'left-6' : 'left-1'}`} />
                </button>
                <span className="text-[11px] font-semibold text-slate-600 dark:text-slate-400">{form.is_public ? 'Public Profile' : 'Private Profile'}</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
              <div className="space-y-6">
                <div>
                  <label className="block text-[11px] font-semibold text-slate-400 mb-2 ml-1 tracking-wider opacity-60">Username</label>
                  <div className="relative group">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 text-sm transition-colors group-focus-within:text-orange-500">@</span>
                    <input
                      type="text" value={form.username}
                      onChange={(e) => setForm({ ...form, username: e.target.value })}
                      className="w-full bg-slate-50/50 dark:bg-white/[0.02] pl-10 pr-4 py-4 rounded-2xl text-sm font-medium border border-slate-100 dark:border-white/5 focus:border-orange-500/30 focus:ring-1 focus:ring-orange-500/10 outline-none text-slate-800 dark:text-white transition-all"
                      placeholder="username"
                    />
                  </div>
                  <p className="mt-2 text-[10px] text-slate-400 font-medium ml-1">Changed {recentChanges.length}/2 times recently</p>
                </div>

                <div>
                  <label className="block text-[11px] font-semibold text-slate-400 mb-2 ml-1 tracking-wider opacity-60">Program</label>
                  <input
                    type="text" value={form.program}
                    onChange={(e) => setForm({ ...form, program: e.target.value })}
                    className="w-full bg-slate-50/50 dark:bg-white/[0.02] px-4 py-4 rounded-2xl text-sm font-medium border border-slate-100 dark:border-white/5 focus:border-orange-500/30 outline-none text-slate-800 dark:text-white transition-all"
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[11px] font-semibold text-slate-400 mb-2 ml-1 tracking-wider opacity-60">Reg. Number</label>
                    <input
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-slate-400 mb-2 ml-1 tracking-wider opacity-60">Batch</label>
                    <input
                ...
                <div>
                  <label className="block text-[11px] font-semibold text-slate-400 mb-2 ml-1 tracking-wider opacity-60">Bio</label>
                  <textarea
                    value={form.bio}
                    onChange={(e) => setForm({ ...form, bio: e.target.value })}
                    className="w-full bg-slate-50/50 dark:bg-white/[0.02] px-4 py-4 rounded-2xl text-sm font-medium border border-slate-100 dark:border-white/5 focus:border-orange-500/30 outline-none text-slate-800 dark:text-white transition-all h-[116px] resize-none"
            ...
            <button
              ...
              className="w-full mt-10 bg-slate-900 dark:bg-white text-white dark:text-black py-4 rounded-2xl font-semibold text-sm transition-all hover:opacity-90 active:scale-[0.98] disabled:opacity-50 border-none flex items-center justify-center gap-2"
            >
          ...
          <div className="glass-panel p-8 md:p-10 rounded-[32px] border border-slate-200 dark:border-white/5 bg-white/50 dark:bg-black/20 backdrop-blur-xl">
            <div className="mb-8">
              <h3 className="text-lg font-semibold text-slate-800 dark:text-white leading-tight">Profile Frames</h3>
              <p className="text-xs text-slate-500 font-medium">Select a frame to showcase your progress</p>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-4">
              ...
              <div 
                ...
                className={`p-4 rounded-2xl border-2 transition-all cursor-pointer flex flex-col items-center justify-center gap-3 relative group ${!userProfile?.avatar_frame ? 'border-orange-500 bg-orange-500/[0.03]' : 'border-slate-100 dark:border-white/5 hover:border-slate-200 dark:hover:border-white/10'}`}
              >
                <div className="w-16 h-16 rounded-full border-2 border-dotted border-slate-200 dark:border-white/10 flex items-center justify-center text-[10px] font-bold text-slate-300 dark:text-slate-600">None</div>
                <span className="text-[10px] font-semibold text-slate-400 group-hover:text-slate-600 dark:group-hover:text-slate-300 transition-colors">Default</span>
              </div>
              ...
              {(userProfile.unlocked_frames || []).map(frame => {
                ...
                return (
                  <div 
                    ...
                    className={`p-4 rounded-2xl border-2 transition-all cursor-pointer flex flex-col items-center justify-center gap-3 relative group ${isActive ? 'border-orange-500 bg-orange-500/[0.03]' : 'border-slate-100 dark:border-white/5 hover:border-slate-200 dark:hover:border-white/10'}`}
                  >
                    <div className="relative w-16 h-16 flex items-center justify-center">
                      <img src={`/Nexus-Journey/${frame}`} alt={frameTitle} className="w-full h-full object-contain z-10" />
                      <div className="absolute inset-2 rounded-full bg-slate-50 dark:bg-white/[0.02]" />
                    </div>
                    <span className={`text-[10px] font-semibold transition-colors ${isActive ? 'text-orange-500' : 'text-slate-400 group-hover:text-slate-600 dark:group-hover:text-slate-300'}`}>{frameTitle}</span>
              ...
              {(!userProfile.unlocked_frames || userProfile.unlocked_frames.length === 0) && (
                <div className="col-span-full py-12 text-center bg-slate-50/50 dark:bg-white/[0.01] rounded-2xl border border-slate-100 dark:border-white/5">
                  <div className="mb-4 flex flex-col items-center">
                    <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-white/5 flex items-center justify-center mb-3">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5 text-slate-400"><rect x="3" y="11" width="18" height="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg>
                    </div>
                    <p className="text-xs font-semibold text-slate-400">Locked Frames</p>
                    <p className="text-[10px] text-slate-500 mt-1 max-w-[200px] mx-auto">Explore the learning journey to unlock unique profile frames</p>
                  </div>
                  <button 
                    onClick={() => navigateToModule(ModuleType.QUIZ)}
                    className="px-6 py-2.5 bg-slate-900 dark:bg-white text-white dark:text-black text-[10px] font-bold rounded-xl transition-all active:scale-[0.98]"
                  >
                    Start Journey
                  </button>
                </div>
      ...
      <div className="flex flex-col items-center pt-8">
        ...
        <p className="mt-8 text-[10px] font-medium text-slate-300 dark:text-slate-700 opacity-40 tracking-wider">Build 2.5a</p>
      </div>
    </div>
  );
};
